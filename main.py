...
async def create_match(data: dict, db: Session = Depends(get_db)):
    ...
    for p in players_list:
        ...
        # 對齊 models.py 欄位：initial_character, final_character, alignment, survived
        performance = models.MatchPlayer(
            match_id=new_match.id,
            player_id=db_player.id,
            initial_character=p.get("initial_character") or p.get("character"),
            final_character=p.get("final_character") or p.get("character"),
...
async def get_player_stats(name: str, db: Session = Depends(get_db)):
    ...
    return {
        ...
        "top_roles": sorted(roles_stat.items(), key=lambda x: x[1], reverse=True)[:5]
    }
...
# 自動掛載靜態資源目錄
# 邏輯：檢查根目錄或 static/ 內部是否存在該資料夾並正確掛載
