(() => {
  ...
  function renderDetail(item) {
    ...
    detail.innerHTML = `
      <header class="script-detail-heading"><div><div class="script-category">${escapeHtml(item.category || '\u5287\u672c')}</div><h2>${escapeHtml(item.name_zh_tw)} <small>${escapeHtml(item.version || '')}</small></h2><div class="script-detail-meta"><span><i class="fa-solid fa-user-pen"></i> ${escapeHtml(item.author_name || '\u4f5c\u8005\u5f85\u88dc')}</span>${tags}</div>${item.tagline ? `<div class="script-tagline">${richText(item.tagline)}</div>` : ''}</div><div class="script-detail-actions"><a class="script-source" href="${escapeHtml(item.json_download_url || `/api/scripts/${item.slug}/download.json`)}" download><i class="fa-solid fa-download"></i> \u4e0b\u8f09\u7e41\u4e2d JSON</a>${item.source_url ? `<a class="script-source" href="${escapeHtml(item.source_url)}" target="_blank" rel="noopener"><i class="fa-solid fa-arrow-up-right-from-square"></i> \u539f\u59cb\u6587\u7ae0</a>` : ''}</div></header>
