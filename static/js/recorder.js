...
const S2T_MAP = {
  ...
  '卖':'賣', '讯':'訊', '杂':'雜', '发':'髮', '门':'門', '间':'間', '关':'關', '頭':'頭', '风':'風', '飞':'飛',
  '鸟':'鳥', '龙':'龍', '馬':'馬', '魚':'魚', '雞':'雞', '陰':'陰', '陽':'陽', '東':'東', '西':'西', '視':'視',
...
const MASTER_ROLE_DB = [
  ...
  {id:"hatter",name:"帽匠",team:"outsider",firstNight:0,otherNight:800,otherNightReminder:"在當晚，一同喚醒所有爪牙和惡魔。對他們展示“該角色的能力對你生效”信息標記，然後展示帽匠角色標記。每名以此被喚醒的玩家可以選擇搖頭或指向角色列表上與自己當前角色類型相同的一個角色。如果一名玩家在做出自己的選擇後會使得自己的角色與一名其他玩家選擇的角色相同，對他搖頭示意他重新選擇。在選擇完成後讓他重新入睡。移除“今晚茶會”提示標記。根據這些玩家的選擇，在魔典上執行相應的角色變化操作。",image:"https://oss.gstonegames.com/data_file/clocktower/upload/202312/c_9723021341071_86da6df4.jpg"},
  ...
  {id:"lil_monsta",name:"小怪寶",team:"demon",firstNight:4400,otherNight:7300,firstNightReminder:"喚醒所有爪牙，允許他們以指向的方式決定誰照看小怪寶，但不能產生其他交流，否則會有非常糟糕的事情發生。",otherNightReminder:"喚醒所有爪牙，允許他們以指向的方式決定誰照看小怪寶，但不能產生其他交流，否則會有非常糟糕的事情發生。說書人選擇一名玩家，那名玩家死亡。",image:"https://oss.gstonegames.com/data_file/clocktower/role_icon/lil_monsta.png"},
...
const getRoleInputConfig = (role) => {
  ...
  if (isRole(['washerwoman', 'librarian', 'investigator'], ['洗衣婦', '圖書管理員', '圖書館員', '調查員'])) return [
  ...
  if (isRole(['cerenovus', 'pit-hag', 'kazali', 'grandmother', 'widow', 'puzzlemaster', 'jiaohuazi'], ['洗腦師', '麻臉巫婆', '卡扎力', '祖母', '寡婦', '解謎大師', '叫花子'])) return [
    { key: 'p1', type: 'player', label: '目標玩家' },
    { key: 'r1', type: 'role', label: '對應角色' }
  ];
  if (isRole(['chef', 'empath', 'mathematician', 'shiguan', 'fangshi', 'clockmaker', 'oracle', 'juggler', 'dagengren', 'chambermaid'], ['廚師', '共情者', '數學家', '史官', '方士', '鐘錶匠', '神諭者', '雜耍藝人', '打更人', '侍女'])) return [
    { key: 'num', type: 'number', label: '得知數字' }
  ];
  if (isRole(['fortune_teller', 'seamstress'], ['占卜師', '女裁縫'])) return [
    { key: 'p1', type: 'player', label: '玩家 1' },
    { key: 'p2', type: 'player', label: '玩家 2' },
  ...
  if (isRole(['al-hadikhia', 'dianyuzhang', 'yinluren'], ['哈迪寂亞', '典獄長', '引路人'])) return [
    { key: 'p1', type: 'player', label: '玩家 1' },
    { key: 'p2', type: 'player', label: '玩家 2' },
    { key: 'p3', type: 'player', label: '玩家 3' }
  ];
  if (isRole(['shabaloth', 'harpy', 'qianke', 'evil_twin', 'dianxiaoer', 'knight', 'baojun', 'innkeeper', 'barber', 'xuncha', 'jianning'], ['沙巴洛斯', '鷹身女妖', '掮客', '鏡像雙子', '店小二', '騎士', '暴君', '旅店老闆', '理髮師', '巡察', '奸佞'])) return [
    { key: 'p1', type: 'player', label: '玩家 1' },
    { key: 'p2', type: 'player', label: '玩家 2' }
  ];
  if (isRole(['qintianjian', 'fengshuishi', 'shugenja', 'gudiao'], ['欽天監', '風水師', '修行者', '蠱雕'])) return [
    { key: 'dir', type: 'select', label: '方向', options: ['左/順時針', '右/逆時針', '相同'] }
  ];
  if (isRole(['langzhong', 'niangjiushi'], ['郎中', '釀酒師'])) return [
    { key: 'p1', type: 'player', label: '目標對象' },
    { key: 'word', type: 'text', label: '提示詞/信息' }
  ];
  if (isRole(['zhen', 'xionghaizi', 'ojo', 'courtier', 'pixie', 'alchemist', 'daoke', 'bianlianshi'], ['鴆', '熊孩子', '奧赫', '侍臣', '小精靈', '煉金術士', '刀客', '變臉師'])) return [
          ...
          {configs.map(c => (
            <div key={c.key}>
              {c.type === 'player' ? (
                ...
                /* 🟢 核心修正：根據 type 支援 text, number */
                <input 
                  type={c.type || "text"} 
                  placeholder={c.label} 
                  value={formData[c.key] || ""}
                  onChange={e=>setFormData({...formData, [c.key]: e.target.value})} 
                  className="w-full bg-slate-800 border border-slate-700 rounded-lg text-xs p-1.5 text-slate-200 outline-none" 
                />
              )}
            </div>
          ))}
...
const App = () => {
  ...
  const allAvailableRoles = useMemo(() => {
    ...
    travellers.forEach(tr => { if (!combined.some(r => r.id === tr.id)) combined.push(tr); });
    return combined;
  }, [script]);
  ...
  const recordEvent = (actor, action, target, detail) => {
    ...
    setLogs(prevLogs => {
      ...
      if (phaseIndex >= 0) newLogs[phaseIndex].events.push(newEvent);
  ...
  const advancePhase = () => {
    ...
    const snapshotDetail = players.map(p => {
      ...
      return `[${p.id}號] ${p.isDead ? '💀 死亡' : '存活'} - (${roleDisplay}) ${p.name}`;
    }).join('\n');
    recordEvent('系統', '狀態快照', '全體玩家', snapshotDetail);
  ...
  const handleScriptUpload = (event) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        const meta = rawRoles.find(i => i.id === '_meta');
        if (meta?.name) setScriptName(meta.name);
        const parsed = rawRoles.map(item => {
          ...
          return { id: roleId, name: dbRole?.name || toTraditional(item.name || ""), team: dbRole?.team || item.team, firstNight: dbRole?.firstNight || 0, otherNight: dbRole?.otherNight || 0, firstNightReminder: dbRole?.firstNightReminder || "", otherNightReminder: dbRole?.otherNightReminder || "", image: dbRole?.image || "", ability: dbRole?.ability || "" };
        }).filter(Boolean);
        setScript(parsed);
        showAlert("劇本載入成功！");
      } catch (err) { showAlert("載入失敗，請檢查格式。"); }
    };
  ...
  const dayActors = useMemo(() => {
    ...
    return players.filter(p => p.role && DAY_ACTION_ROLES.some(r => p.role.id.includes(r) || p.role.name.includes(r)));
  }, [players, gamePhase]);

  return (
    <div className="h-full bg-slate-950 text-slate-200 flex flex-col relative overflow-hidden">
      {modalConfig.isOpen && (
        <div className="fixed inset-0 bg-black/80 z-[9999] flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl p-6 max-w-sm w-full shadow-2xl flex flex-col gap-4">
            <p className="text-sm font-bold">{modalConfig.message}</p>
            <div className="flex justify-end gap-3">
              {modalConfig.type === 'confirm' && <button onClick={closeModal} className="px-4 py-2 bg-slate-800 rounded-xl text-xs">取消</button>}
              <button onClick={() => { if(modalConfig.onConfirm) modalConfig.onConfirm(); closeModal(); }} className="px-4 py-2 bg-indigo-600 rounded-xl text-xs">確定</button>
            </div>
          </div>
        </div>
      )}
      ...
      <header className="bg-slate-900 border-b border-slate-800 p-4 flex justify-between items-center z-20">
        <div className="flex items-center gap-3">
          <h1 className="text-lg font-black flex items-center gap-2">📖 血染覆盤記錄器</h1>
          <button onClick={handleResetClick} className="text-[10px] bg-red-900/30 text-red-400 px-2 py-1 rounded border border-red-800">🔄 重置</button>
        </div>
        <div className="flex items-center gap-3">
          <label className="bg-slate-800 px-3 py-1.5 rounded-xl cursor-pointer text-xs font-bold border border-slate-700">📜 載入劇本<input type="file" onChange={handleScriptUpload} className="hidden" /></label>
          <div className={`px-4 py-1.5 rounded-xl border text-xs font-black ${gamePhase.type === 'Setup' ? 'bg-slate-800' : 'bg-indigo-900/40 text-indigo-300'}`}>
            {gamePhase.type === 'Setup' ? '設置' : gamePhase.type === 'Prep' ? '準備' : `第 ${gamePhase.number} ${gamePhase.type === 'Night' ? '夜' : '天'}`}
          </div>
          <button onClick={advancePhase} className="bg-indigo-600 px-4 py-1.5 rounded-xl font-black text-xs shadow-lg">▶️ 下個階段</button>
        </div>
      </header>

      <div className="bg-[#0f172a] border-b border-white/10 px-4 py-2 flex flex-wrap gap-4 text-xs">
          <input type="text" value={scriptName} onChange={e=>setScriptName(e.target.value)} className="bg-slate-900 border border-slate-700 rounded p-1" placeholder="劇本名" />
          <input type="date" value={gameDate} onChange={e=>setGameDate(e.target.value)} className="bg-slate-900 border border-slate-700 rounded p-1" />
          <select value={gameLocation} onChange={e=>setGameLocation(e.target.value)} className="bg-slate-900 border border-slate-700 rounded p-1">
            <option value="線上 (Discord)">線上 (Discord)</option><option value="台北">台北</option><option value="其他">其他...</option>
          </select>
      </div>

      <div className="bg-slate-900/50 p-4 border-b border-slate-800 overflow-x-auto">
        <div className="flex gap-3">
          {players.map(p => (
            <div key={p.id} className={`shrink-0 w-24 p-2 rounded-2xl border-2 text-center ${p.isDead ? 'bg-slate-950 border-slate-800 opacity-60' : 'bg-slate-800 border-indigo-500/20'}`}>
              <button onClick={() => setSelectingRoleFor(p.id)} className="w-10 h-10 rounded-full bg-slate-900 flex items-center justify-center mx-auto mb-1">
                {p.role ? <img src={p.role.image || `https://oss.gstonegames.com/data_file/clocktower/role_icon/${p.role.id}.png`} className="w-8 h-8 object-contain" /> : "➕"}
              </button>
              <div className="text-[10px] font-bold text-slate-400 truncate">{p.role?.name || "未指派"}</div>
              <input value={p.name} onChange={e=>setPlayers(players.map(pl=>pl.id===p.id?{...pl, name:e.target.value}:pl))} className="w-full bg-transparent text-[10px] text-center outline-none" />
              {gamePhase.type !== 'Setup' && <button onClick={()=>setPlayers(players.map(pl=>pl.id===p.id?{...pl, isDead:!pl.isDead}:pl))} className="text-[10px] mt-1">💀</button>}
            </div>
          ))}
          {gamePhase.type === 'Setup' && <button onClick={()=>setPlayers([...players, {id:players.length+1, name:`玩家 ${players.length+1}`, role:null, isDead:false}])} className="p-4 bg-slate-800 rounded-2xl border-2 border-dashed border-slate-700 text-slate-500 text-xs">＋ 玩家</button>}
        </div>
      </div>

      <main className="flex-1 flex flex-col lg:flex-row overflow-hidden">
        <div className="flex-1 p-4 lg:p-6 overflow-y-auto custom-scrollbar border-r border-slate-800 bg-[#020617]">
          {gamePhase.type === 'Prep' ? (
            <div className="space-y-6">
              <h2 className="font-black text-purple-400">準備階段 - 指派隱藏身分</h2>
              <div className="flex gap-2">
                <select value={hiddenTarget} onChange={e=>setHiddenTarget(e.target.value)} className="bg-slate-800 border border-slate-700 rounded p-2 text-xs">
                  <option value="">選擇玩家...</option>
                  {players.map(p=><option key={p.id} value={p.id}>{p.id}號 {p.name}</option>)}
                </select>
                <input value={hiddenValue} onChange={e=>setHiddenValue(e.target.value)} className="bg-slate-800 border border-slate-700 rounded p-2 text-xs" placeholder="實際身分 (如: 酒鬼)" />
                <button onClick={()=>{
                  if(!hiddenTarget || !hiddenValue) return;
                  setPlayers(players.map(p=>p.id==hiddenTarget?{...p, hiddenRole:hiddenValue}:p));
                  recordEvent('系統','指派身分',players.find(p=>p.id==hiddenTarget).name, `實際身分:${hiddenValue}`);
                  setHiddenTarget(""); setHiddenValue("");
                }} className="bg-purple-600 px-4 rounded text-xs">記錄</button>
              </div>
            </div>
          ) : (
            <div className="space-y-6">
              <div className="space-y-4">
                {(gamePhase.type === 'Night' ? wakingOrder : dayActors).map(p => (
                  <RoleActionCard 
                    key={p.id} player={p} 
                    reminder={gamePhase.type==='Night' ? (gamePhase.number===1?p.role.firstNightReminder:p.role.otherNightReminder) : p.role.ability}
                    onRecord={(a,t,d)=>recordEvent(p.role.name, a, t, d)}
                    players={players} script={script}
                  />
                ))}
              </div>

              {/* 🟢 深夜/日間 通用自由紀錄區 */}
              {(gamePhase.type === 'Night' || gamePhase.type === 'Day') && (
                <div className="bg-slate-900/50 border border-slate-800 p-6 rounded-[2rem] space-y-4 mt-8">
                  <h3 className="text-sm font-black text-slate-400 flex items-center gap-2">📝 {gamePhase.type==='Night'?'深夜':'自由'}事件紀錄</h3>
                  <div className="grid grid-cols-2 gap-4">
                    <input type="text" value={dayAction.actor} onChange={e => setDayAction({...dayAction, actor: e.target.value})} className="bg-slate-800 border border-slate-700 rounded-xl px-4 py-2 text-sm" placeholder="發起人" />
                    <input type="text" value={dayAction.action} onChange={e => setDayAction({...dayAction, action: e.target.value})} className="bg-slate-800 border border-slate-700 rounded-xl px-4 py-2 text-sm" placeholder="動作 (如: 重新入場)" />
                    <input type="text" value={dayAction.target} onChange={e => setDayAction({...dayAction, target: e.target.value})} className="bg-slate-800 border border-slate-700 rounded-xl px-4 py-2 text-sm" placeholder="目標" />
                    <input type="text" value={dayAction.detail} onChange={e => setDayAction({...dayAction, detail: e.target.value})} className="bg-slate-800 border border-slate-700 rounded-xl px-4 py-2 text-sm" placeholder="備註/細節" />
                  </div>
                  <button onClick={() => {
                    recordEvent(dayAction.actor || '系統', dayAction.action || '特殊事件', dayAction.target || '無', dayAction.detail || '無細節');
                    setDayAction({ actor: "", action: "事件紀錄", target: "", detail: "" });
                  }} className="w-full bg-slate-700 font-black py-2 rounded-xl text-xs">寫入紀錄</button>
                </div>
              )}
            </div>
          )}
        ...
        <div className="w-full lg:w-96 bg-slate-950/50 border-l border-slate-800 flex flex-col p-4">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-xs font-black text-slate-500 uppercase tracking-widest">時間軸紀錄</h3>
            <button onClick={exportHistory} className="bg-indigo-900/30 text-indigo-400 px-3 py-1 rounded-lg text-[10px] font-bold border border-indigo-800">📥 匯出文字檔</button>
          </div>
          <div className="flex-1 overflow-y-auto space-y-4 custom-scrollbar">
            {logs.map((phase, idx) => (
              <div key={idx} className="border-l-2 border-slate-800 pl-4 py-2">
                <div className="text-[10px] font-black text-indigo-400 mb-2">{phase.phase}</div>
                {phase.events.map(e => (
                  <div key={e.id} className="bg-slate-900/50 p-2 rounded mb-2 border border-slate-800 text-[11px]">
                    <div className="flex justify-between opacity-50 mb-1"><span>{e.time}</span><span>{e.actor}</span></div>
                    <div><span className="text-indigo-300">{e.action}</span> → {e.target}</div>
                    {e.detail && e.detail!=="無備註" && <div className="mt-1 pl-2 border-l border-slate-700 text-slate-400 text-[10px] whitespace-pre-wrap">{e.detail}</div>}
                    <button onClick={()=>setLogs(logs.map((ph,i)=>i===idx?{...ph, events:ph.events.filter(ev=>ev.id!==e.id)}:ph))} className="text-red-900 hover:text-red-500 float-right">✕</button>
                  </div>
                ))}
              </div>
            ))}
      ...
      {selectingRoleFor && (
        <div className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center p-6 backdrop-blur-md">
          <div className="bg-slate-900 w-full max-w-4xl max-h-[80vh] rounded-3xl border border-slate-700 flex flex-col overflow-hidden shadow-2xl">
            <div className="p-4 border-b border-slate-800 flex justify-between items-center">
              <h2 className="font-bold">選擇角色</h2>
              <input placeholder="搜尋..." value={searchTerm} onChange={e=>setSearchTerm(e.target.value)} className="bg-slate-800 rounded-xl px-4 py-1.5 text-xs outline-none focus:border-indigo-500" />
              <button onClick={()=>setSelectingRoleFor(null)} className="text-slate-500">❌</button>
            </div>
            <div className="flex-1 overflow-y-auto p-4 grid grid-cols-2 md:grid-cols-4 gap-3">
              {allAvailableRoles.filter(r=>r.name.includes(searchTerm)||r.id.includes(searchTerm)).map(r => (
                <button key={r.id} onClick={()=>{
                  setPlayers(players.map(p=>p.id===selectingRoleFor?{...p, role:r}:p));
                  setSelectingRoleFor(null);
                }} className="bg-slate-800 p-2 rounded-xl border border-slate-700 flex flex-col items-center hover:bg-indigo-600/20 transition-all">
                  <img src={r.image || `https://oss.gstonegames.com/data_file/clocktower/role_icon/${r.id}.png`} className="w-8 h-8 mb-1" />
