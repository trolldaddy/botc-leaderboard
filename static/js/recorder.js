...
const S2T_MAP = {
  ...
  '护':'護', '帮':'幫', '戏':'戲', '脸':'臉', '说':'說', '书':'書', '這':'這', '么':'麼', '没':'沒', '樣':'樣',
  '唤':'喚', '醒':'醒', '择':'擇', '两':'兩', '个':'個', '当':'當', '会':'會', '从':'從', '来':'來', '让':'讓',
  '记':'記', '为':'為', '该':'該', '复':'復', '隐':'隱', '离':'離', '异':'異', '钟':'鐘', '表':'錶', '梦':'夢',
  '卖':'賣', '訊':'訊', '杂':'雜', '发':'髮', '门':'門', '间':'間', '关':'關', '頭':'頭', '风':'風', '飞':'飛',
  ...
  '罌':'罌', '氣':'氣', '駛':'駛', '築':'築', '头':'頭', '马':'馬', '鱼':'魚', '鸡':'雞', '阴':'陰', '阳':'陽',
  '视':'視', '妇':'婦', '术':'術', '药':'藥', '农':'農', '长':'長', '护':'護', '帮':'幫', '戏':'戲', '说':'說',
  '书':'書', '这':'這', '头':'頭', '长':'長'
};
...
const MASTER_ROLE_DB = [
  ...
  {id:"imp",name:"小惡魔",team:"demon",firstNight:0,otherNight:4900,otherNightReminder:"讓小惡魔選擇一名玩家。標記那名玩家死亡。",image:"https://oss.gstonegames.com/data_file/clocktower/role_icon/imp.png"},
...
const getRoleInputConfig = (role) => {
  ...
  if (isRole(['shabaloth', 'harpy', 'qianke', 'evil_twin', 'dianxiaoer', 'knight', 'baojun', 'innkeeper', 'barber', 'xuncha', 'jianning'], ['沙巴洛斯', '鷹身女妖', '掮客', '鏡像雙子', '店小二', '騎士', '暴君', '旅店老闆', '旅店老板', '理髮師', '理發師', '巡察', '奸佞', '鹰身女妖', '镜像双子', '騎士', '理发师'])) return [
    { key: 'p1', type: 'player', label: '玩家 1' },
    { key: 'p2', type: 'player', label: '玩家 2' }
  ];
...
const RoleActionCard = ({ player, reminder, onRecord, players, script }) => {
  ...
  const handleRecord = () => {
    ...
    configs.forEach(c => {
      ...
      else if (c.key === 'p3' && targetStr) targetStr += ` & ${val}`;
  ...
  return (
    <div className="h-full bg-slate-950 text-slate-200 flex flex-col relative overflow-hidden font-sans">
                ...
                {players.map(p => (
                    <div key={p.id} className={`p-3 rounded-2xl border-2 text-center relative transition-all ${p.isDead ? 'bg-slate-950 grayscale border-slate-800' : 'bg-slate-800 border-indigo-500/20 shadow-lg'}`}>
                        <button onClick={()=>setSelectingRoleFor(p.id)} className="w-12 h-12 rounded-full bg-slate-950 mx-auto mb-2 flex items-center justify-center overflow-hidden border border-slate-700">
                            {p.role ? <img src={p.role.image} className="w-10 h-10 object-contain" /> : <span className="opacity-20">+</span>}
                        </button>
      ...
      <main className="flex-1 flex flex-col lg:flex-row overflow-hidden bg-[#020617]">
        <div className="flex-1 p-6 overflow-y-auto custom-scrollbar border-r border-slate-800 shadow-2xl">
          {gamePhase.type === 'Prep' ? (
            <div className="space-y-6">
                ...
                <div className="bg-purple-950/20 border border-purple-900/30 p-5 rounded-3xl space-y-4">
                    <h3 className="text-xs font-black text-purple-400">身分標記 (如:酒鬼)</h3>
                    <div className="flex gap-3">
                        ...
                        <input value={hiddenValue} onChange={e=>setHiddenValue(e.target.value)} className="bg-slate-900 border border-purple-900/40 rounded-xl p-2 text-xs flex-1" placeholder="隱藏身分" />
                        <button onClick={()=>{ if(!hiddenTarget||!hiddenValue)return; setPlayers(players.map(p=>p.id==hiddenTarget?{...p, hiddenRole:hiddenValue}:p)); recordEvent("系統", "身分", players.find(p=>p.id==hiddenTarget).name, `實際身分:${hiddenValue}`); setHiddenTarget(""); setHiddenValue(""); }} className="bg-purple-600 px-6 rounded-xl text-xs font-black">記錄</button>
                    </div>
                </div>
            </div>
          ) : (
            <div className="space-y-6">
              ...
              <div className="bg-slate-900/50 border border-slate-800 p-6 rounded-[2rem] space-y-4 shadow-inner mt-8">
                ...
                <button onClick={()=>{ recordEvent(dayAction.actor||'系統', dayAction.action||'記錄', dayAction.target||'無', dayAction.detail||''); setDayAction({actor:'', action:'自由記錄', target:'', detail:''}); }} className="w-full bg-slate-700 hover:bg-slate-600 text-white font-black py-2.5 rounded-xl text-xs shadow-lg transition-all">寫入日誌</button>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="w-full lg:w-[400px] bg-slate-950/50 p-6 overflow-y-auto border-l border-slate-800 flex flex-col relative">
          <div className="flex justify-between items-center mb-6 sticky top-0 bg-slate-950/80 backdrop-blur py-2 z-10">
            ...
            <button onClick={exportHistory} className="text-[10px] text-indigo-400 font-bold hover:underline">匯出文字檔</button>
          </div>
          <div className="space-y-6 pb-20">
            {logs.slice().reverse().map((phaseLog, idx) => (
              <div key={idx} className="relative border-l border-slate-800 pl-6 py-2 animate-fadeIn">
                <div className="absolute -left-1 top-2 w-2 h-2 rounded-full bg-indigo-500 shadow-[0_0_10px_rgba(99,102,241,0.5)]"></div>
                <div className="text-[10px] font-black text-indigo-400 mb-4 uppercase tracking-widest">{phaseLog.phase}</div>
                <div className="space-y-3">
                  {phaseLog.events.slice().reverse().map(e => (
                    <div key={e.id} className="bg-slate-900/80 border border-slate-800 p-3 rounded-xl group relative hover:border-slate-700 transition-all">
        ...
        <div className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center p-6 backdrop-blur-xl animate-fadeIn">
          <div className="bg-slate-900 w-full max-w-4xl max-h-[85vh] rounded-[2.5rem] border border-slate-700 flex flex-col overflow-hidden shadow-2xl">
            <div className="p-6 border-b border-slate-800 flex justify-between items-center gap-4">
              <h2 className="font-black text-indigo-300">選擇劇本角色</h2>
              <input placeholder="搜尋角色名稱..." onChange={e=>setSearchTerm(e.target.value)} className="bg-slate-800 rounded-xl px-4 py-2 text-sm outline-none focus:ring-2 focus:ring-indigo-500 text-white flex-1" />
              <button onClick={()=>setSelectingRoleFor(null)} className="text-slate-400 hover:text-white transition-colors">✕</button>
            </div>
            <div className="flex-1 overflow-y-auto p-8 grid grid-cols-2 md:grid-cols-5 gap-4 custom-scrollbar">
              {MASTER_ROLE_DB.filter(r=>r.name.includes(searchTerm)).map(r => (
                <button key={r.id} onClick={()=>{ setPlayers(players.map(p=>p.id===selectingRoleFor?{...p, role:r}:p)); setSelectingRoleFor(null); setSearchTerm(""); }} className="bg-slate-800/50 p-4 rounded-3xl border border-slate-700 flex flex-col items-center hover:bg-indigo-600/20 hover:border-indigo-500/50 transition-all group shadow-inner">
