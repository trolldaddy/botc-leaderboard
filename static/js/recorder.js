...
const S2T_MAP = {
    ...
    '记':'記', '為':'為', '该':'該', '复':'復', '隐':'隱', '离':'離', '异':'異', '钟':'鐘', '表':'錶', '梦':'夢',
...
const DAY_ACTION_ROLES = ['slayer', 'gunslinger', 'juggler', 'artist', 'fisherman', 'gossip', 'psychopath', 'golem', 'virgin'];
...
const MASTER_ROLE_DB = [
 {id:"beggar",name:"乞丐",team:"traveller",firstNight:0,otherNight:0,image:"https://oss.gstonegames.com/data_file/clocktower/role_icon/beggar.png"},
      ...
      {id:"snake_charmer",name:"舞蛇人",team:"townsfolk",firstNight:5200,otherNight:2100,firstNightReminder:"讓舞蛇人選擇一名玩家。如果舞蛇人選中了惡魔：展示“你是”信息標記和惡魔角色標記。用拇指向下代表他陣營變為邪惡。在魔典中交換舞蛇人和惡魔的角色標記。讓原來的舞蛇人重新入睡。喚醒原來的惡魔。對原惡魔展示“你是”信息標記和舞蛇人角色標記，並用拇指向上代表他陣營變為善良，並標記這名玩家中毒。",otherNightReminder:"讓舞蛇人選擇一名玩家。如果舞蛇人選中了惡魔：展示“你是”信息標記和惡魔角色標記。用拇指向下代表他陣營變為邪惡。在魔典中交換舞蛇人和惡魔的角色標記。讓原來的舞蛇人重新入睡。喚醒原來的恶魔。对原恶魔展示“你是”信息标記和舞蛇人角色标記，并用拇指向上代表他阵营变为善良，并标記这名玩家中毒。",image:"https://oss.gstonegames.com/data_file/clocktower/role_icon/snake_charmer.png"},
...
const RoleActionCard = ({ player, reminder, onRecord, players, script }) => {
    ...
    const handleRecord = () => {
        ...
        configs.forEach(c => {
            ...
            else if (['p2', 'p3'].includes(c.key)) targetStr += ` & ${val}`;
    ...
    return (
        <div className={`bg-slate-900 border p-4 rounded-3xl flex flex-col md:flex-row gap-4 transition-all ${player.isDead ? 'border-red-900/50 opacity-70' : 'border-slate-700'}`}>
            <div className="flex flex-col items-center justify-center gap-1 md:w-32 shrink-0 border-b md:border-b-0 md:border-r border-slate-700/50 pb-4 md:pb-0 md:pr-4">
                <div className="relative">
                    <img src={imgUrl} className="w-14 h-14 object-contain filter drop-shadow-lg" alt="" onError={(e)=>e.target.src='https://oss.gstonegames.com/data_file/clocktower/role_icon/unknown.png'} />
                    <div className="absolute -top-1 -right-1 w-5 h-5 bg-slate-800 text-slate-300 text-[10px] font-black rounded-full flex items-center justify-center border border-slate-600">{player.id}</div>
                </div>
                <h3 className="font-black text-sm text-indigo-100 text-center">{player.role.name}</h3>
                <span className="text-[10px] text-slate-500 font-bold">{player.name}</span>
            </div>
            <div className="flex-1 flex flex-col justify-between gap-3">
                <div className="text-[11px] text-slate-400 bg-black/30 p-2 rounded-xl border-l-2 border-indigo-500/50 italic">{reminder}</div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-1">
                    {configs.map(c => (
                        <div key={c.key} className="flex flex-col">
                            <label className="text-[9px] font-black text-slate-500 uppercase ml-1">{c.label}</label>
                            {c.type === 'player' ? (
                                <select value={formData[c.key] || ""} onChange={e => setFormData({...formData, [c.key]: e.target.value})} className="bg-slate-800 border border-slate-700 rounded-lg px-2 py-1.5 text-xs text-slate-200">
                                    <option value="">選擇玩家...</option>
                                    {players.filter(p => p.name).map(p => <option key={p.id} value={p.name}>{p.id} - {p.name}</option>)}
                                </select>
                            ) : c.type === 'role' ? (
                                <select value={formData[c.key] || ""} onChange={e => setFormData({...formData, [c.key]: e.target.value})} className="bg-slate-800 border border-slate-700 rounded-lg px-2 py-1.5 text-xs text-slate-200">
                                ...
                                <input type={c.type} value={formData[c.key] || ""} onChange={e => setFormData({...formData, [c.key]: e.target.value})} className="bg-slate-800 border border-slate-700 rounded-lg px-2 py-1.5 text-xs text-slate-200" />
                            )}
                        </div>
                    ))}
                </div>
                <div className="flex gap-2 mt-2">
                    <input type="text" value={detailNote} onChange={e => setDetailNote(e.target.value)} className="flex-1 bg-slate-800 border border-slate-700 rounded-lg px-3 py-1.5 text-xs text-slate-200" placeholder="備註..." />
                    <button onClick={handleRecord} className={`px-4 py-1.5 rounded-lg text-xs font-black transition-all ${isRecorded ? 'bg-emerald-600 text-white' : 'bg-indigo-600 hover:bg-indigo-500 text-white'}`}>
                        {isRecorded ? '✅ 已紀錄' : '寫入'}
                    </button>
                </div>
            </div>
...
const App = () => {
    ...
    const [players, setPlayers] = useState(() => loadState('botc_players', Array.from({ length: 12 }, (_, i) => ({ id: i + 1, name: `玩家 ${i + 1}`, role: null, hiddenRole: "", isDead: false }))));
    ...
    useEffect(() => {
        ...
        localStorage.setItem('botc_playerCount', JSON.stringify(playerCount));
    ...
    }, [script, playerCount, players, gamePhase, logs]);
    ...
    const recordEvent = (actor, action, target, detail) => {
        ...
        setLogs(prev => {
            const newLogs = [...prev];
            const idx = newLogs.findIndex(l => l.phase === phaseLabel);
            if (idx >= 0) newLogs[idx].events.unshift(newEvent);
    ...
    const wakingOrder = useMemo(() => {
        ...
        return (gamePhase.number === 1 ? inPlay.filter(p => p.role.firstNight > 0) : inPlay.filter(p => p.role.otherNight > 0));
    }, [players, gamePhase]);
        ...
        <div className="h-full flex flex-col bg-slate-950">
            <header className="p-4 border-b border-slate-800 flex justify-between items-center bg-slate-900/50">
                <div className="flex items-center gap-3">
                    <h1 className="font-black text-white text-sm">血染覆盤</h1>
                    <span className="text-[10px] bg-indigo-900 text-indigo-300 px-2 py-0.5 rounded">{gamePhase.type} P{gamePhase.number}</span>
                </div>
                <div className="flex gap-2">
                    <button onClick={() => setGamePhase(p => ({...p, type: p.type === 'Night' ? 'Day' : 'Night', number: p.type === 'Day' ? p.number + 1 : p.number}))} className="px-4 py-1.5 bg-indigo-600 text-white rounded-lg text-xs font-black">下一階段 ▶️</button>
                </div>
            </header>

            <main className="flex-1 flex overflow-hidden">
                <div className="w-1/3 border-r border-slate-800 p-4 overflow-y-auto custom-scrollbar">
                    <div className="grid grid-cols-2 gap-2">
                        {players.map(p => (
                            <div key={p.id} className={`p-2 rounded-xl border ${p.isDead ? 'bg-slate-900 opacity-50' : 'bg-slate-800'}`}>
                                <div onClick={() => setSelectingRoleFor(p.id)} className="w-8 h-8 mx-auto cursor-pointer">
                                    {p.role ? <img src={p.role.image} className="object-contain" /> : '❓'}
                                </div>
                                <input className="w-full bg-transparent text-[10px] text-center outline-none" value={p.name} onChange={e => setPlayers(players.map(pl => pl.id === p.id ? {...pl, name: e.target.value} : pl))} />
                            </div>
                        ))}
                    </div>
                </div>
                ...
                <div className="flex-1 p-4 overflow-y-auto custom-scrollbar">
                    {gamePhase.type === 'Night' ? wakingOrder.map(p => (
                        <RoleActionCard key={p.id} player={p} onRecord={(ac, ta, de) => recordEvent(p.role.name, ac, ta, de)} players={players} script={script} />
                    )) : <p className="text-center text-slate-600">日間紀錄模式...</p>}
                </div>
            </main>

            {selectingRoleFor && (
                <div className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center p-10">
                    <div className="bg-slate-900 w-full max-w-2xl h-full rounded-3xl p-6 overflow-y-auto">
                        <div className="grid grid-cols-4 gap-2">
                            {MASTER_ROLE_DB.map(r => (
                                <button key={r.id} onClick={() => { setPlayers(players.map(p => p.id === selectingRoleFor ? {...p, role: r} : p)); setSelectingRoleFor(null); }} className="p-2 bg-slate-800 rounded-xl flex flex-col items-center">
                                    <img src={r.image} className="w-8 h-8" />
