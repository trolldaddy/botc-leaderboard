const RoleActionCard = ({ player, reminder, onRecord, players, script }) => {
  ...
  return (
    <div className={`bg-[#0f172a] border p-4 sm:p-5 rounded-3xl flex flex-col md:flex-row gap-4 sm:gap-5 transition-all ${player.isDead ? 'border-red-900/50 opacity-70' : 'border-slate-700'}`}>
      <div className="flex flex-col items-center justify-center gap-1 md:w-32 shrink-0 border-b md:border-b-0 md:border-r border-slate-700/50 pb-4 md:pb-0 md:pr-4">
        <div className="relative">
          <img src={imgUrl} className="w-16 h-16 object-contain filter drop-shadow-lg mb-1" alt="" onError={(e)=>e.target.style.display='none'} />
        ...
        <h3 className="font-black text-base text-indigo-100 text-center leading-tight">{player.role.name}</h3>
        <span className="text-xs text-slate-400 font-bold text-center">{player.name}</span>
      ...
      <div className="flex-1 flex flex-col justify-between gap-3">
        <div>
          {reminder && (
            <div className="text-[11px] text-slate-400 bg-black/30 p-2.5 rounded-xl border-l-2 border-indigo-500/50 italic leading-relaxed">
              提醒：{reminder}
            </div>
          )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-3">
              {configs.map((config) => (
                <div key={config.key} className="flex flex-col">
                  <label className="text-[9px] font-black text-slate-500 uppercase mb-1 ml-1">{config.label}</label>
                  
                  {config.type === 'player' && (
                    <select 
                      value={formData[config.key] || ""} 
                      onChange={(e) => handleInputChange(config.key, e.target.value)}
                      className="bg-slate-800 border border-slate-700 rounded-xl px-2 py-2 text-xs outline-none focus:border-indigo-500 text-slate-200"
                    >
                      <option value="">選擇玩家...</option>
                      {players.filter(p => p.name).map(p => (
                        <option key={p.id} value={p.name}>{p.id} 號 - {p.name} {p.role ? `(${p.role.name})` : ''}</option>
                      ))}
                    </select>
                  )}
                  ...
                  {config.type === 'number' && (
                    <input 
                      ...
                      value={formData[config.key] || ""}
                      onChange={(e) => handleInputChange(config.key, e.target.value)}
                      className="bg-slate-800 border border-slate-700 rounded-xl px-3 py-2 text-xs outline-none focus:border-indigo-500 text-slate-200"
                      placeholder="輸入數字..."
                    />
                  )}
        ...
        <div className="flex gap-2 items-stretch mt-1 pt-3 border-t border-slate-800/50">
          <input 
            type="text"
            value={detailNote}
            onChange={(e) => setDetailNote(e.target.value)}
            onKeyPress={(e) => e.key === 'Enter' && handleRecord()}
            className="flex-1 bg-slate-800 border border-slate-700 rounded-xl px-3 py-2 text-xs outline-none focus:border-indigo-500 text-slate-200"
            placeholder="結果 / 備註 (自由填寫)..."
          />
          <button 
            onClick={handleRecord}
            className={`flex items-center justify-center gap-1.5 px-4 py-2 rounded-xl text-xs font-black transition-all shrink-0 ${isRecorded ? 'bg-emerald-600 text-white' : 'bg-indigo-600 hover:bg-indigo-500 text-white active:scale-95'}`}
          >
            {isRecorded ? <span>✅ 已紀錄</span> : '寫入日誌'}
          </button>
        </div>
      </div>
    </div>
...
// ==========================================
// 本地存檔 Wrapper (支援 Vercel 原生 LocalStorage)
// ==========================================
...
const App = () => {
  ...
  // 即時存檔
  useEffect(() => { saveState('botc_script', script); }, [script]);
  useEffect(() => { saveState('botc_playerCount', playerCount); }, [playerCount]);
      ...
      try {
        ...
        const parsedRoles = rawRoles
          .map(item => {
            ...
            // 呼叫外部的轉繁體與資料庫對接
            const tName = typeof toTraditional === 'function' ? toTraditional(item.name || "") : (item.name || "");
            const tAbility = typeof toTraditional === 'function' ? toTraditional(item.ability || "") : (item.ability || "");

            let dbRole = (typeof MASTER_ROLE_DB !== 'undefined' ? MASTER_ROLE_DB : []).find(r => r.id === roleId);
  ...
  const recordEvent = (actor, action, target, detail) => {
    ...
    setLogs(prevLogs => {
      ...
      if (phaseIndex >= 0) {
        newLogs[phaseIndex].events.push(newEvent); // 推到最後面 (上至下)
      } else {
  ...
  return (
    <div className="h-full bg-[#020617] text-slate-200 font-sans flex flex-col relative overflow-hidden rounded-xl">
      ...
      {/* 頂部操作工具列 */}
      <div className="bg-[#0f172a] border-b border-slate-800 px-4 py-3 shrink-0 flex flex-wrap items-center justify-between gap-3 z-20">
      ...
      <div className="bg-[#0f172a] border-b border-slate-800 p-3 shrink-0 flex items-center justify-between z-10 shadow-md relative">
      ...
      <div className="bg-[#0f172a]/50 border-b border-slate-800 p-4 shrink-0 shadow-inner z-10 w-full overflow-y-auto max-h-[30vh] custom-scrollbar">
      ...
      {selectingRoleFor !== null && (
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-fadeIn">
          <div className="bg-[#0f172a] rounded-3xl w-full max-w-5xl max-h-[85vh] border border-slate-700 flex flex-col shadow-2xl">
            ...
            <div className="flex-1 overflow-y-auto p-6 custom-scrollbar">
              {script.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-slate-500 gap-4">
                  ...
                  <p>請先點擊上方載入劇本 JSON 檔案</p>
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                  {['townsfolk', 'outsider', 'minion', 'demon', 'traveller'].map(team => {
                    const teamRoles = script.filter(r => r.team === team && (r.name.includes(searchTerm) || r.id.toLowerCase().includes(searchTerm.toLowerCase())));
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          {teamRoles.map(role => (
                            <button 
                              ...
                              className="flex flex-col items-center p-3 bg-slate-800 hover:bg-indigo-600/30 rounded-xl border border-slate-700 transition-all group"
                            >
      ...
      <main className="flex-1 flex flex-col lg:flex-row overflow-hidden min-h-0 bg-[#020617]">
        ...
        <div className="flex-1 min-h-0 min-w-0 p-4 lg:p-6 overflow-y-auto custom-scrollbar border-b lg:border-b-0 lg:border-r border-slate-800">
          {gamePhase.type === 'Setup' ? (
            <div className="h-full flex flex-col items-center justify-center text-slate-500 space-y-4 py-10">
              ...
              <p className="text-xs text-center">分配完成後點擊右上角的「進入準備」。</p>
            </div>
          ) : gamePhase.type === 'Prep' ? (
            ...
            <div className="space-y-6">
              ...
              <div className="bg-[#0f172a] border border-slate-700 p-6 rounded-[2rem] shadow-xl space-y-4">
              ...
              <div className="bg-[#0f172a]/50 border border-slate-800 p-6 rounded-[2rem] shadow-inner space-y-4">
        ...
        <div className="flex-1 min-h-0 min-w-0 bg-[#0f172a]/50 p-4 lg:p-6 overflow-y-auto custom-scrollbar flex flex-col relative border-l border-slate-800">
          <div className="flex justify-between items-center mb-6 sticky top-0 bg-[#0a0f1d] py-2 z-10 px-2 rounded-lg">
                      ...
                      {editingLog?.id === event.id ? (
                        ...
                        <>
                          <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 flex items-center gap-1 transition-opacity">
                            <button onClick={() => startEditLog(idx, event)} className="p-1.5 text-slate-400 hover:text-indigo-400 bg-slate-800 hover:bg-slate-700 rounded-md transition-colors"><span className="text-xs">✏️</span></button>
                            <button onClick={() => deleteLog(idx, event.id)} className="p-1.5 text-slate-400 hover:text-red-400 bg-slate-800 hover:bg-slate-700 rounded-md transition-colors"><span className="text-xs">🗑️</span></button>
                          </div>
