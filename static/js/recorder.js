...
const RoleActionCard = ({ player, reminder, onRecord, players, script }) => {
  ...
  return (
    <div className={`bg-slate-900 border p-4 sm:p-5 rounded-3xl flex flex-col md:flex-row gap-4 sm:gap-5 transition-all ${player.isDead ? 'border-red-900/50 opacity-70' : 'border-slate-700'}`}>
                ...
                <div key={config.key} className="flex flex-col">
                  ...
                  {config.type === 'role' && (
                    ...
                    >
                      ...
                      {script.map(r => (
                        <option key={r.id} value={r.name}>{r.name} ({r.team === 'townsfolk' ? '鎮民' : r.team === 'outsider' ? '外來者' : r.team === 'minion' ? '爪牙' : r.team === 'demon' ? '惡魔' : '旅行者'})</option>
                      ))}
                    </select>
                  )}
...
const App = () => {
  ...
  const dayActors = useMemo(() => {
    ...
    return players.filter(p => p.role && (
      DAY_ACTION_ROLES.includes(p.role.baseRoleId) || 
  ...
  return (
    <div className="h-full bg-slate-950 text-slate-200 font-sans flex flex-col relative overflow-hidden">
      ...
      <main className="flex-1 flex flex-col lg:flex-row overflow-hidden min-h-0">
        ...
        <div className="flex-1 min-h-0 min-w-0 bg-slate-950/50 p-4 lg:p-6 overflow-y-auto custom-scrollbar flex flex-col relative">
          <div className="flex justify-between items-center mb-6 sticky top-0 bg-slate-950 py-2 z-10">
