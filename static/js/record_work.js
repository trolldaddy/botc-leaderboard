...

{
    const initRecord = async () => {
        ...
        if (!window.MASTER_ROLE_DB || window.MASTER_ROLE_DB.length === 0) {
            console.error("❌ 警告：未偵測到角色數據庫，陣營自動判定將失效。");
        }
        ...
        // 2. 同步基礎數據
        await refreshPlayerDatalist(); // 載入玩家建議名單
        loadRecentMatches();           // 載入左側最近紀錄
    ...
    const refreshPlayerDatalist = async () => {
        ...
        try {
            ...
            if (resp.ok) {
                ...
                console.log(`✅ 已同步 ${names.length} 名已知玩家`);
            }
        } catch (err) {
            ...
            container.innerHTML = recent.map(m => {
                ...
                return `
                    <div class="match-item">
                        <span class="script-name">${m.script}</span>
                        <div class="meta-info">
                            <span><i class="fa-solid fa-location-dot"></i> ${m.location || '未知'}</span>
                            <span style="color: ${winColor}; font-weight: bold;">${winLabel}</span>
                        </div>
                        <div style="font-size: 0.65rem; color: var(--text-muted); margin-top: 4px;">錄入日期：${dateStr}</div>
                    </div>
                `;
            }).join('');
        } catch (err) {
    ...
    window.autoFillFromLog = () => {
        ...
        if (dateMatch) document.getElementById('match-date').value = dateMatch[1]; // 🟢 自動解析日期
