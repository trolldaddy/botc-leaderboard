...

{
    ...
    const initHistory = async () => {
        ...
        try {
            if (container) {
                container.innerHTML = `<div style="text-align:center; padding:5rem; color:var(--text-muted);"><i class="fa-solid fa-spinner fa-spin"></i> 正在翻閱歷史魔典...</div>`;
            }
    ...
    const applyLogic = () => {
        ...
        if (!currentKeyword) {
            ...
            filtered = allMatches.filter(m => {
                ...
                const matchLocation = (m.location || "").toLowerCase().includes(currentKeyword);
                const matchST = (m.storyteller || "").toLowerCase().includes(currentKeyword);
    ...
    const updateStatsUI = (matches) => {
        ...
        if (total === 0) {
            ...
            document.getElementById('stat-top-location').innerText = "無";
    ...
    const renderHistoryList = (matches) => {
        ...
        if (matches.length === 0) {
            container.innerHTML = `<div style="text-align:center; color:var(--text-muted); padding:4rem;">找不到相關對局</div>`;
            return;
        }
        ...
        container.innerHTML = matches.map(m => {
            ...
            // 🟢 處理玩家名單，將其轉為簡潔字串
            const playerNamesStr = m.players ? m.players.map(p => p.player_name).join('、') : "尚無玩家數據";
                    ...
                    <div class="match-main-row" onclick="toggleMatchDetails(${m.id})">
                        ...
                        <div class="match-info-content">
                            ...
                            <div class="info-row-bottom">
                                <i class="fa-solid fa-id-card-clip" style="font-size:0.7rem; color:var(--accent-gold); opacity:0.7;"></i>
